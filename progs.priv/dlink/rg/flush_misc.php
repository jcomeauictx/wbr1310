...
if ($wanif != "" && $wanip != "")
{
	$cmd_head="iptables -t nat -A PRE_MISC -i ".$wanif;
	$cmd_reply=" -p icmp --icmp-type echo-reply";
	$cmd_request=" -d ".$wanip." -p icmp --icmp-type echo-request";
	...
	if (query("/security/firewall/httpallow") == 1)
	{
		...
		{ echo $cmd_head." -p tcp --dport ".$httpremoteport." -j REDIRECT --to-ports 80\n"; }
		else
		{ echo $cmd_head." -p tcp --dport ".$httpremoteport." -s ".$httpremoteip." -j REDIRECT --to-ports 80\n"; }
