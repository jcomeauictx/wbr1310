...
$bridgemode	= query("/function/bridgemode");
$Result		= "";
$NewIPAddress	= "";

if ($IsAccessPoint=="true")
{
	if ($bridgemode!="1")		{ $Result="ERROR"; /* we only support router mode. */ }
	else if (query("/bridge")!="1")	{ $Result="REBOOT"; set("/bridge", "1"); }
	else				{ $Result="OK"; /* we are already in bridge mode. */ }
}
else
{
	if ($bridgemode!="1")		{ $Result="OK"; /* we only support router mode. */ }
	else if (query("/bridge")=="1")	{ $Result="REBOOT"; set("/bridge", "0"); }
