...
static size_t output2buffer(int fd, char * buffer, size_t size)
{
	fd_set read_set;
	size_t read_count = 0;
	size_t rn;
	...
	for (;;)
	{
		...
		if (FD_ISSET(fd, &read_set))
		{
			rn = read(fd, &buffer[read_count], size-read_count);
			if (rn <= 0) break;
			read_count += rn;
			if (buffer[read_count-1] == '\0') break;
		}
	}
	return read_count;
}
...
int lrgdb_do_ephp(const char * phpfile, char * buffer, size_t size)
{
	int fd = lrgdb_open(NULL);
	int read_count;

	if (fd)
	{
		if (send_xmldb_cmd(fd, RGDB_EPHP, 0, phpfile, strlen(phpfile)+1) < 0)
		{
			lrgdb_close(fd);
			return -1;
		}
		read_count = output2buffer(fd, buffer, size);
		lrgdb_close(fd);
		return read_count;
	}
	return -1;
