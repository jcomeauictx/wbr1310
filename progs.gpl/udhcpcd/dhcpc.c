...
{
	DEBUG(LOG_INFO, "entering %s listen mode", new_mode ? (new_mode == 1 ? "kernel" : "raw") : "none");
	close(fd);
	fd = -1;
...
#ifndef LOGNUM
	syslog(ALOG_NOTICE|LOG_DEBUG,"DHCP: Client performing a DHCP renew.");
#else
	syslog(ALOG_NOTICE|LOG_DEBUG,"NTC:016");
#endif
	switch (state)
	{
	case BOUND:
...
#ifndef LOGNUM
		syslog(ALOG_NOTICE, "DHCP: Client release IP %s to server %s.", inet_ntoa(temp_addr), buffer);
#else
		syslog(ALOG_NOTICE, "NTC:017[%s][%s]", inet_ntoa(temp_addr), buffer);
#endif
...
#endif
{
	...
	while (1)
	{
		...
		c = getopt_long(argc, argv, "c:fbH:h:i:np:qr:s:vdD:R:S:", arg_options, &option_index);
		if (c == -1) break;
	...
	{
		now = time(0);
		DEBUG(LOG_INFO, "timeout = %d, now = %d\n", timeout, now);
		...
		if (tv.tv_sec > 0)
		{
			DEBUG(LOG_INFO, "Waiting on select...(timeout in %d secs)\n", tv.tv_sec);
		...
		DEBUG(LOG_INFO, "Return from select ... (ret = %d)\n", retval);
...
#ifndef LOGNUM
					syslog(ALOG_NOTICE|LOG_DEBUG, "DHCP: Client send DISCOVER.");
#else
					syslog(ALOG_NOTICE|LOG_DEBUG, "NTC:010");
#endif

					/* timeout = now + ((packet_num == 2) ? 4 : 2); */
					timeout = ((unsigned int)client_config.discover_delay) << packet_num;
					if (timeout > 64) timeout = 64;
					DEBUG(LOG_INFO, "packet_num=%d, timeout = %u\n", packet_num, timeout);
					timeout += now;
					packet_num++;
				}
				else
...
#ifndef LOGNUM
					syslog(ALOG_NOTICE|LOG_DEBUG,"DHCP: Client send REQUEST, Request IP %s from %s.", ipstr1, ipstr2);
#else
					syslog(ALOG_NOTICE|LOG_DEBUG,"NTC:012[%s][%s]", ipstr1, ipstr2);
#endif
					free(ipstr1);
					free(ipstr2);
...
#ifndef LOGNUM
						syslog(ALOG_NOTICE|LOG_DEBUG, "DHCP: Client receive OFFER from %s.", inet_ntoa(temp_addr));
#else
						syslog(ALOG_NOTICE|LOG_DEBUG, "NTC:011[%s]", inet_ntoa(temp_addr));
#endif
			...
			case REBINDING:
				...
				if (*message == DHCPACK)
				{
					...
					if (state == REQUESTING)
					{
						...
						{
							LOG(LOG_INFO, "the address (%s) is used by another host\n", inet_ntoa(temp_addr));
#ifndef LOGNUM
							syslog(ALOG_NOTICE|LOG_DEBUG,"DHCP: Client receive ACK, but the address (%s) is used by another host.", inet_ntoa(temp_addr));
#else
							syslog(ALOG_NOTICE|LOG_DEBUG,"NTC:014[%s]", inet_ntoa(temp_addr));
#endif
							send_decline(xid);
							state = INIT_SELECTING;
							timeout = now;
					...
					ipstr2 = strdup(inet_ntoa(temp_addr));
#ifndef LOGNUM
					syslog(ALOG_NOTICE|LOG_DEBUG,"DHCP: Client receive ACK from %s, IP=%1, Lease time=%ld.",ipstr1, ipstr2, lease);
#else
					syslog(ALOG_NOTICE|LOG_DEBUG,"NTC:013[%s][%s][%ld]", ipstr1, ipstr2, lease);
#endif
					free(ipstr1);
					free(ipstr2);
...
#ifndef LOGNUM
					syslog(ALOG_NOTICE|LOG_DEBUG,"DHCP: Client receive NAK.");
#else
					syslog(ALOG_NOTICE|LOG_DEBUG,"NTC:015");
#endif
