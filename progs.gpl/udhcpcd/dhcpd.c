...
#ifndef LOGNUM
			syslog(ALOG_NOTICE|LOG_DEBUG, "DHCP: Server receive DISCOVER from %s.", mac);
#else
			syslog(ALOG_NOTICE|LOG_DEBUG, "NTC:018[%s]", mac);
#endif
		...
		case DHCPREQUEST:
...
#ifndef LOGNUM
			syslog(ALOG_NOTICE|LOG_DEBUG, "DHCP: Server receive REQUEST from %s.", mac);
#else
			syslog(ALOG_NOTICE|LOG_DEBUG, "NTC:022[%s]", mac);
#endif

#if 0 //Joy masked: hostname is moved to sendACK()
			...
			if (lease)	/*ADDME: or static lease */
			{
				...
				else
				{
					...
					{
						/* INIT-REBOOT State */
						if (lease->yiaddr == requested_align) sendACK(&packet, lease->yiaddr);
						else sendNAK(&packet);
					}
					else
					{
						/* RENEWING or REBINDING State */
						if (lease->yiaddr == packet.ciaddr) sendACK(&packet, lease->yiaddr);
						else sendNAK(&packet); /* don't know what to do!!!! */
					}						
				}
