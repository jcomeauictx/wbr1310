...
void
jsw_create_psk_conf(struct hostapd_data *hapd)
{
	struct jsw_profile *jp;
	int i;
	FILE *f_p1, *f_p2;
	...
	*p = '\0';
	...
	fname_p2 = JSW_P2_CONFIG;	
	f_p1 = fopen(fname_p1, "r");
	f_p2 = fopen(fname_p2, "w");
	if (f_p1 == NULL || f_p2 == NULL) {
		printf("Could not open configuration files.%s\n", 
				JSW_P2_CONFIG);
		return;
	}
