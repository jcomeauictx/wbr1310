...
jswP1Failed(struct jsw_session *js, jsw_event_t event)
{
	...
	wpa_printf(MSG_DEBUG, "P1 failed, event %d\n", event);
	hapd->jsw_profile->p1_in_progress = 0;
	jswSetLEDPattern(&ready);

	hostapd_helper("enter_p1");
	js_free_station(sta);
}
...
jswP2Failed(struct jsw_session *js, jsw_event_t event)
{
	...
	wpa_printf(MSG_DEBUG, "P2 failed, event %d\n", event);
...
jswP1Successful(struct jsw_session *js)
{
	...
	wpa_printf(MSG_DEBUG, "P1 successful\n");	
	...
	jsw_create_psk_conf(hapd);
	js_free_station(sta);

	printf("JUMPSTART: hostapd is about to reload\n\n\n\n");
	pid = getpid();
	kill(pid, SIGHUP);
}
