...
#define JSW_P2_CONFIG		  "/var/run/hostapd.conf"
...
#if 0
/* XXX Divy. test the info values */
#define jswMsgOk(_len, _eapol, _vp, _proto, _keyHdr, _info, _data_len)	   \
	(((_len) >= jswMinLen()) 					&& \
	((_eapol)->version <= EAPOL_VERSION) 				&& \
	((_eapol)->type ==  IEEE802_1X_TYPE_EAP_PACKET) 		&& \
	((_vp)->type == JSW_EAP_VP_TYPE)				&& \
	(!(memcmp((_vp)->smiOID, eapSmiOID, sizeof(eapSmiOID))))	&& \
	((_proto)->proto ==  ATHL2P_JUMPSTART_PROTO)			&& \
	((_proto)->version <= JSW_VERSION)				&& \
	((_keyHdr)->type == JSW_KEY_DESC)				&& \
	(_data_len < JSW_MAX_KEYDATA_LEN)		&& \
	(((_info) & WPA_KEY_INFO_TYPE_MASK) == JSW_KEY_INFO_TYPE))
#endif
...
void jswP2GetMicKey(struct jsw_session *jp, u8 *micKey);
void jswP2GetMic2Key(struct jsw_session *jp, u8 *micKey);
void jswSessionDestroy();
void js_create_ssid(struct hostapd_data *hapd, const char *pref);
void jsw_create_psk_conf(struct hostapd_data *hapd);
/*
 * jswAuthSm.c
 */
void jswP2Start(struct jsw_session *jp, jsw_event_t event);
void jswP1Start(struct jsw_session *jp, jsw_event_t event);
int jswIsPassPending(struct jsw_session *jp);
void jswTimerStart(struct jsw_session *js, int timer);
void jswTimerStop(struct jsw_session *js);
/*
 * jswMsg.c
 */
int jswSendPMKAck(struct jsw_session *jp);
int jswSendPMK(struct jsw_session *jp);
int jswP2SendJdsOffer(struct jsw_session *jp);
int jswSendJdsPassAck(struct jsw_session *jp);
int jswSendJdsAck(struct jsw_session *jp);
int jswP1SendJdsOffer(struct jsw_session *jp);
int jswGetTlvs(struct jsw_session *jp, struct ieee802_1x_hdr *eapolHdr,
	       JSW_FRAME_OPS op);
int jswSendPassReq(struct jsw_session *jp);
