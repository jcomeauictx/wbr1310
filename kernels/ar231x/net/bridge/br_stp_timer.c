 ...
 *
 *	$Id: //depot/sw/linuxsrc/src/kernels/mips-linux-2.4.25/net/bridge/br_stp_timer.c#1 $
 *
...
static void br_forward_delay_timer_expired(struct net_bridge_port *p)
{
	if (p->state == BR_STATE_LISTENING) {
		...
		br_timer_set(&p->bwctrl_timer, jiffies);
#endif
	}
}
...
#ifdef CONFIG_BRIDGE_MULTICAST_BWCTRL
	if ((p->bandwidth != 0) && br_timer_has_expired(&p->bwctrl_timer, 1*HZ))
	{
		br_timer_clear(&p->bwctrl_timer);
		p->accumulation = 0;
		br_timer_set(&p->bwctrl_timer, jiffies);
	}
#endif
